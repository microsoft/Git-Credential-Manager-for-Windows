...
namespace Microsoft.Alm.CredentialHelper
{
    class Program
    {
        ...
        private static void PrintHelpMessage()
        {
            ...
            Console.Out.WriteLine("                  If a path is provided, the package will not be deployed");
            Console.Out.WriteLine("                  into Git's libexec folder.");
            Console.Out.WriteLine();
            Console.Out.WriteLine("   --git-path     Specifies a path to the preferred instance of Git.");
            Console.Out.WriteLine("                  If a path is provided, only the specified instance will");
            Console.Out.WriteLine("                  be updated.");
            Console.Out.WriteLine();
            Console.Out.WriteLine("   --unattend     Instructs the installer to not prompt the user for input");
            Console.Out.WriteLine("                  during installation.");
            Console.Out.WriteLine();
            Console.Out.WriteLine("   --ignore-netfx Instructs the installer to not check the installed version");
            Console.Out.WriteLine("                  of Microsoft .Net Framework against the minimum required.");
            Console.Out.WriteLine("                  version (4.5.1).");
            Console.Out.WriteLine();
